...
from convert_comfyui_utils import (
    import_custom_nodes,
    find_path,
...
class FileHandler:
    ...
    def write_code_to_file(file_path: str | TextIO, code: str) -> None:
        ...
        Returns:
            None
        """
        print(f"DEBUG: STARTING WRITE CODE TO FILE")
...
class LoadOrderDeterminer:
    ...
    def _load_special_functions_first(self) -> None:
        ...
        for key in self.data:
            # print(f"DEBUG: Key = {key}, Type = {type(self.data[key])}, Value = {self.data[key]}")
            # print(f"DEBUG: class_type = {self.data[key].get('class_type')}")
            # print(f"DEBUG: Available Node Mappings: {list(self.node_class_mappings.keys())}")
...
class CodeGenerator:
    ...
    def generate_workflow(
        ...
        print("DEBUG: Generated code before formatting:\n", final_code)
    ...
    def assemble_python_code(
        ...
        """
        # Get the source code of the Comfyui_utils functions as a string
        func_strings = []
        for func in [
            get_value_at_index,
            find_path,
        ...
        print(f"DEBUG: non PEP8 final_code: \n {final_code} ")
...
class ComfyUItoPython:
    ...
    def execute(self):
        ...
        Returns:
            None
        """
        base_node_mapping = len(self.base_node_class_mappings.keys())
        node_mapping = len(self.node_class_mappings.keys())
        ...
        new_node_mapping = len(self.node_class_mappings.keys())
        ...
        # NEW: Convert "nodes" array into the dict structure
        # node_dict = {}
        # if "nodes" in data and isinstance(data["nodes"], list):
        #     for node in data["nodes"]:
        #         node_id = str(node["id"])
        #         node_type = node["type"]  # rename it to "class_type"
        #         # Build a minimal dict with what the script expects, e.g. "class_type", "inputs"
        #         node_dict[node_id] = {
        #             "class_type": node_type,
        #             "inputs": {},  # you'd parse node["inputs"] as needed
        #             # ...
        #         }
        #     # Replace data with node_dict
        #     data = node_dict
